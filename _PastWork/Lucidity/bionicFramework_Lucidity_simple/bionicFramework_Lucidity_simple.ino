...
int numSamplesColor = 10;
float accelAngleRaw;
SmoothInput accelAngle = SmoothInput(numSamplesColor);
float rx = -0.5;
float ry = -0.5;
float rz = 0.0;
...
float decayCoeff = 0.15;
float decayDeccelerator = 0;
...
//    Serial.println(sine[i]);
  ...
  event_ind = t.every( 30, fireAnts );
  t.every( 100, incHueNow );
}
...
//  mySerial.println("R255");
  ...
  accel.update();
  accelMagRaw = sqrt(pow(accel.x(),2)+pow(accel.y(),2)+pow(accel.z(),2))-256;
  if ( accelMagRaw < 0 ) accelMagRaw = 0;
  accelMag.smooth(accelMagRaw);
  Serial.println(accelMag.getSmooth());
...
//  Serial.println(valNow);
  if ( valNow > decay && valNow > 25) {
    decay = valNow;
    decayCounter = 0;
    decayDeccelerator = min(25/valNow, 1);
    Serial.println(valNow);
    if ( millis() - messageTimer > messageInterval ) {
      if ( decay > 40 ) {
        mySerial.print(hueNow);
        mySerial.print(",");
        if (decay < 60) {
          mySerial.print(1);
        }
    ...
    decayCounter++;
    decayDeccelerator = min (1,25/decay);
    decay-=decayCoeff*pow(decayCounter,2)*decayDeccelerator;
  }
  if ( decay < 0 ) {
    decay = 0;
    decayCounter = 0;
...
void powerBar() {
  int i;
  valNow = map(constrain(accelMag.getSmooth(),0,512),0,512,0,strip.numPixels()-1);
...
//  valNow = map(constrain(accelMag.getSmooth(),0,512),0,512,0,255);
//  if ( valNow > decay ) {
//    if( valNow-decay > colorBuffer && valNow > 100 ) {
//      huePow = hueNow;
...
//    decay = valNow;
//    decayCounter = 0;
////    if( millis() - colorTimer > colorInterval && valNow-decay > colorBuffer) {
//  }
//  else {
//    decayCounter++;
