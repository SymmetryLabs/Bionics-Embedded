...
float rz = 0.0;
long colorTimer;
long colorInterval = 200;
//int colorBuffer = 30;
int colorCutoff = 75;

float decay = 0;
float decayCounter = 0;
float decayCoeff = 0.05;
float decayAccelerator = 0;
...
//  mySerial.println("R255");
  ...
  accelAngleRaw =  float((rx*accel.x() + ry*accel.y() + rz*accel.z()))/512;
  accelAngle.smooth( constrain(constrain(accelAngleRaw, 0, 1)*359, 0, 359) );
  hueNow = accelAngle.getSmooth();
...
//  Serial.println(valNow);
  if ( valNow > decay ) {
    if( valNow > colorCutoff ) {
      decayAccelerator = 100/valNow;
    ...
    decayCounter++;
    decayAccelerator = max (1, decayAccelerator);
    decay-=decayCoeff*pow(decayCounter,2)*decayAccelerator;
  }
  if ( decay < 10 ) {
    decay = 0;
    decayCounter = 0;
